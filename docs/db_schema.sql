...
-- ============================================================
-- v2: blocked_queue 테이블 (Phase A 차단 감지 → WeedBot 연동)
-- ============================================================
CREATE TABLE IF NOT EXISTS public.blocked_queue (
  id           bigint generated by default as identity primary key,
  article_id   bigint unique references public.article(id) on delete set null,
  link         text not null,
  title        text null,
  block_reason text null,  -- http_403 / title: "..." / no_content_elements / fetch_error
  status       text not null default 'PENDING',  -- PENDING / DONE / SKIPPED
  created_at   timestamptz not null default now(),
  updated_at   timestamptz not null default now()
) tablespace pg_default;

CREATE INDEX IF NOT EXISTS idx_blocked_queue_status ON blocked_queue(status);
CREATE INDEX IF NOT EXISTS idx_blocked_queue_created ON blocked_queue(created_at);
